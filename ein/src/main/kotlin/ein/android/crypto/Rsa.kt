...
internal object Rsa{
    ...
    private fun init(){
        if(isInited) return
        isInited = true
        val alias = "${eApp.packName}.rsakeypairs"
    ...
    private fun initL(alias: String) = if(Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR2) false
    else try{
        with(KeyPairGenerator.getInstance("RSA", KEY_PROVIDER_NAME)){
            ...
            val builder = KeyPairGeneratorSpec.Builder(eApp.app)
                .setAlias(alias)
            ...
            val spec = if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                builder.setKeySize(KEY_LENGTH_BIT).build()
            } else {
                builder.build()
            }
            initialize(spec)
