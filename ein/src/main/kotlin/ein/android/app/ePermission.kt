...
class Permission(internal val act: AppCompatActivity, private val code:Int){
    ...
    fun permissions(vararg arg: String) = arg.forEach {
        if(!ePermission.dangers.contains(it)) throw Exception("invalid permission:$it")
        permissions.add(it)
    }
    fun request() {
        ...
        if(
            if(permissions.isEmpty()) true
                ...
                var cnt = 0
                permissions.forEach{
                    if(ePermission.isGranted == ContextCompat.checkSelfPermission(act, it)) cnt++
                    else notPermitted.add(it)
                }
                cnt == permissions.size
            }
        ){
            act.onRequestPermissionsResult(code, permissions.toTypedArray(), IntArray(permissions.size) {ePermission.isGranted})
            return
