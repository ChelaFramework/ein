...
class eJsonObject:MutableMap<String, eValue> by mutableMapOf(), eValue {
    override val v:MutableMap<String, eValue> = this
    operator fun invoke(k:eString) = invoke(k.v.split("."))
    operator fun invoke(key:String) = invoke(key.split("."))
    operator fun invoke(key:List<String>)= key.fold(this as eValue){target, k->
        when(target){
